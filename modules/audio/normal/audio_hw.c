...
static int set_i2s_parameter(struct mixer *i2s_mixer,int bt_type);
...
static int start_bt_sco_output_stream(struct tiny_stream_out *out)
{
    ...
    */
    ...
    if(adev->bluetooth_type == VX_WB_SAMPLING_RATE)
        pcm_config_btsco = pcm_config_btscoplayback_wb;
    else
        pcm_config_btsco = pcm_config_btscoplayback_nb;

    ALOGI("%s set i2s para samplerate:%d", __func__, adev->bluetooth_type);
    set_i2s_parameter(adev->i2s_mixer, adev->bluetooth_type);

    out->pcm_bt_sco = pcm_open(card, port, PCM_OUT| PCM_MMAP |PCM_NOIRQ | PCM_MONOTONIC, &pcm_config_btsco);
        ...
        size_t bytes)
{
    ...
    if(out->pcm_bt_sco) {
       ...
       if(adev->ext_contrl->dump_info->dump_bt_sco){
        ...
        ALOGI("out_write_bt_sco  bt_type:%d, out_frames:%d, out_byte:%d", adev->bluetooth_type, out_frames, out_bytes);
        ret = pcm_mmap_write(out->pcm_bt_sco, (void *)buf, out_bytes);
#ifdef AUDIO_DUMP_EX
    dump_info.buf = buf;
    dump_info.buf_len = out_bytes;
...
static int set_i2s_parameter(struct mixer *i2s_mixer,int bt_type)
{
    ...
    if (bt_type == VX_WB_SAMPLING_RATE) {
        ...
        ctl = mixer_get_ctl_by_name(i2s_mixer, "i2s_bus_mode");
            mixer_ctl_set_value(ctl, 0, 1);
    }else if (bt_type== VX_NB_SAMPLING_RATE) {
        ctl = mixer_get_ctl_by_name(i2s_mixer, "fs");
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
        ...
        if(checkvalid && val != adev->fm_volume)
        {
            ...
            if (adev->fm_type != AUD_FM_TYPE_LINEIN_3GAINS) {
                if(!adev->fm_mute){
                    ...
                    ALOGE("adev_set_parameters fm_open: %d, fm_volume_tbl[val] %x", adev->fm_open, val, gain);
                    usleep(10000);
                    if(val !=0) {
                        if (adev->private_ctl.fm_da0_mux)
                            mixer_ctl_set_value(adev->private_ctl.fm_da0_mux, 0, 1);
    ...
    if(ret >= 0){
        ...
        pthread_mutex_lock(&adev->lock);
        if (adev->fm_type == AUD_FM_TYPE_LINEIN_3GAINS &&
            !adev->cache_mute && adev->call_start == 0)
                codec_lowpower_open(adev, true);
        pthread_mutex_unlock(&adev->lock);
        ...
        pthread_mutex_lock(&adev->lock);
        if (adev->fm_type == AUD_FM_TYPE_LINEIN_3GAINS &&
            !adev->master_mute && adev->call_start == 0)
            codec_lowpower_open(adev, false);
        pthread_mutex_unlock(&adev->lock);
    ...
    if (ret > 0) {
        ...
        if(val == 1){
            ...
            ret = adev_record_tone_start(adev);
        } else if (val == 0) {
...
int adev_record_tone_start(struct tiny_audio_device *adev)
{
    struct tiny_stream_out *out;
    int ret = 0;
    int card = -1;
    ...
    cp_mixer_enable(adev->record_tone_info, adev->cp_mixer[adev->cp_type], card);
