...
#endif
	} else if (strnicmp(command, COUNTRY, strlen(COUNTRY)) == 0) {
		int skip = strlen(COUNTRY) + 1;
		printkd("CMD: %s\n", command);
		if (10 != skip + 2) {
			ret = -1;
			printkd("%s command error: %s", __func__, command);
		} else {
			printkd("set country code: %s \n", command + skip);
			ret = regulatory_hint(g_wlan.wiphy, command + skip);
		}
	}else {
...
static int wlan_core_thread(void *data)
{
	...
	do
	{
		...
		for(vif_id = NETIF_0_ID; vif_id < WLAN_MAX_ID; vif_id++)
		{
			...
			if (msg_num(msg_q) > 0)
			{
				ret = wlan_tx(tx_fifo, msg_q, msg_num(msg_q));
				if (ret > 0) {
					done += ret;
					thread->done_tx += ret;
				}
			      ...
			      unsigned long event, void *ptr)
{
	...
	case NETDEV_UP:
		...
		if (vif->mode == ITM_NONE_MODE)
		{
			printkd("netdev[%d] = ITM_NONE_MODE, schd later\n", vif_id);
			len = sizeof(ifa->ifa_address);
			event_info = event_alloc(len);
			if (event_info == NULL)
			{
				WARN_ON(1);
				printkd("ALLOC NETDEV EVENT failed\n");
				return NOTIFY_DONE;
			}

			event_info->data_len = len;
			event_info->event_type = WIFI_CMD_GET_IP;

			memcpy(event_info->event_data, &ifa->ifa_address, len);

			spin_lock_bh(&vif->ework.lock);
			list_add_tail(&event_info->list, &vif->ework.head);
			spin_unlock_bh(&vif->ework.lock);
		}
