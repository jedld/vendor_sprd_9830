...
static void cfg80211_schd_send_event(struct event_work *w, struct event_info * e)
{
	...
	while (1) {
		...
		switch(info->event_type)
		{
			...
			case WIFI_CMD_GET_IP:
				wlan_cmd_get_ip(vif->id, info->event_data);
				break;
...
struct event_info *event_alloc(size_t size)
{
	struct event_info *info;
	size_t total_size;
...
void cfg80211_report_scan_frame(unsigned char vif_id, unsigned char *pData, int len)
{
	...
	if( 0 == event->ops)
	{
		...
		mgmt = (struct ieee80211_mgmt *)msa;
		is_probe_resp = ieee80211_is_probe_resp(mgmt->frame_control);
		...
		for(i=0; i < MAX_SCAN_FRAME_BUF_NUM; i++)
		{
			...
			if(event->signal < scan_buf->signal)
				return;
			if (scan_buf->is_probe_resp != is_probe_resp)
				continue;
		...
		for(i=0; i < MAX_SCAN_FRAME_BUF_NUM; i++)
		{
			...
			scan_buf->channel  = event->channel;
			scan_buf->signal   = event->signal;
			scan_buf->is_probe_resp = is_probe_resp;
			//scan_buf->keep = 1;
			scan_buf->live = 1;
	...
	if (0 == current_bss_on && ITM_CONNECTED == vif->cfg80211.connect_status)
	{
			bss = cfg80211_get_bss(wiphy, NULL,
				   vif->cfg80211.bssid,
				   vif->cfg80211.ssid,
			...
			if (bss) {
				...
				itm_bss = cfg80211_inform_bss(wiphy, bss->channel, bss->bssid,
						tsf, bss->capability, bss->beacon_interval, bss->ies->data,
						bss->ies->len, bss->signal, GFP_KERNEL);

				if (unlikely(!itm_bss))
					printkd("[%s %d][vid %d] update BSS: %pM failed\n", __func__, __LINE__, vif_id, bss->bssid);
				else
					printk("update BSS: %pM success\n", bss->bssid);

				cfg80211_put_bss(wiphy, bss);
			}
			else
			{
				printkd("Can not find BSSID: %pM from the current bss list\n", vif->cfg80211.bssid);
			}
	}
...
****************/
	if (vif->cfg80211.scan_request)//bouble check it
	{
		...
		{
			/*Fix Bug 623449
			* sta & p2p coex condition, scan results will be flush ater each scan if the flag:NL80211_SCAN_FLAG_FLUSH was set .
			*p2p scan will flush the sta scan results, when sta try to connect a bssid, the candidate was lost.
			*solution: do not flush sta scan results ater p2p scan.
			*/
			if(vif_id == 1)
			{
				vif->cfg80211.scan_request->flags &= ~NL80211_SCAN_FLAG_FLUSH;
			}
...
static int wlan_cfg80211_start_ap(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_ap_settings *info)
{
	...
	if ((vif->mode == ITM_AP_MODE) && (!vif->enable_softap_max_connections))
		wlan_cmd_set_softap_max_connections(vif, vif->max_num_of_clients);
...
static int itm_wlan_change_mode(wlan_vif_t *vif, enum nl80211_iftype type)
{
	...
	if ((0 == g_wlan.hw.device_open) && (g_wlan.hw.rd != NULL))
	{
		printkd("num of rules is %d\n", g_wlan.hw.rd->n_reg_rules);
		rd_size = sizeof(wlan_ieee80211_regdomain) +
			(g_wlan.hw.rd->n_reg_rules) * sizeof(struct ieee80211_reg_rule);
		wlan_cmd_set_regdom(0, (unsigned char *)g_wlan.hw.rd, rd_size);
		g_wlan.hw.device_open = 1;
		kfree(g_wlan.hw.rd);
		g_wlan.hw.rd = NULL;
	}
	...
	if (!work_pending(&vif->ework.work))
		schedule_work(&vif->ework.work);
...
{
	int ret = 0;
	lte_concur_data_t concur_dat;
	unsigned char *lte_dat;

	switch (cmd)
	{
		case LTE_CONCUR_REQ:
			if (copy_from_user(&concur_dat, (unsigned char *)arg, sizeof(concur_dat)))
			{
				printkd("copy_from_user (data size) failed\n");
				return -EFAULT;
			}

			lte_dat = kmalloc(concur_dat.size, GFP_KERNEL);
			if (lte_dat == NULL)
			{
				printkd("malloc mem failed, size = 0x%x\n", concur_dat.size);
				return -ENOMEM;
			}

			if (copy_from_user(lte_dat, (unsigned char *)arg + sizeof(concur_dat.size),
						concur_dat.size))
			{
				printkd("copy_from_user (lte data) failed\n");
				kfree(lte_dat);
				return -EFAULT;
			}

			ret = wlan_cmd_req_lte_concur(0, lte_dat, concur_dat.size);
			if (ret < 0)
			{
...
int mac_addr_cfg(wlan_vif_t *vif, unsigned char vif_id)
{
	...
	if(NETIF_1_ID == vif_id)
	{
		...
		vif->ndev->dev_addr[3] = vif->ndev->dev_addr[3] ^ 0x2;
#endif /* CONFIG_MACH_SAMSUNG */
	}
...
int wlan_vif_init(wlan_vif_t  *vif, int type, const char *name, void *ops)
{
	...
	*net_priv = vif;
	...
	vif->enable_softap_max_connections = false;
	vif->max_num_of_clients = SOFTAP_DEFAULT_MAX_CONNECTIONS;
