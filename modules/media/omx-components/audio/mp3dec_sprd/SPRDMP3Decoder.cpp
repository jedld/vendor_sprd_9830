...
void SPRDMP3Decoder::onQueueFilled(OMX_U32 portIndex) {
    ...
    while ((!inQueue.empty() || mInputEosUnDecode) && !outQueue.empty()) {
        ...
        if(decoderRet != MP3_ARM_DEC_ERROR_NONE) { //decoder error
          ...
          /**
           * SPRD: bug521919, Play special avi video by Gallery, drag the progressbar, it will have
no sound  *@{
           */
            if(decoderRet == MP3_ARM_DEC_ERROR_FRAMELEN)
            {
              notify(OMX_EventError, OMX_ErrorUndefined, decoderRet, NULL);
