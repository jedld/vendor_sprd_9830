			                ...
			                struct setting_cmd_parameter * parm)
{
...
#endif
		{CAMERA_PARAM_ISP_AE_LOCK_UNLOCK,      NULL},
		{CAMERA_PARAM_ISP_3A_LOCK_UNLOCK,      NULL},
		{CAMERA_PARAM_AE_METERING_AREA,        NULL},
		{CAMERA_PARAM_LENS_FACING,             setting_set_lens_facing},
...
#endif
		{SETTING_GET_LENS_FACING,              setting_get_lens_facing},
	...
	if (!cpt || !parm || cmd_type >= SETTING_TYPE_MAX
		...
		CMR_LOGE("param has error cpt 0x%p, camera_id %ld, array_size %d, cmd_type %ld ,CAMERA_ID_MAX %ld, SETTING_TYPE_MAX %ld",
			cpt, parm->camera_id, cmr_array_size(setting_list), cmd_type,CAMERA_ID_MAX,SETTING_TYPE_MAX);
		return -CMR_CAMERA_INVALID_PARAM;
	}
