...
LOCAL uint32_t _ov8825_close_flag(unsigned long param);
...
static uint32_t s_ov8825_sensor_close_flag = 0;
...
static uint32_t _ov8825_set_sensor_close_flag()
{
	uint32_t rtn = SENSOR_SUCCESS;

	s_ov8825_sensor_close_flag = 1;
...
LOCAL uint32_t _ov8825_access_val(unsigned long param)
{
	...
	switch(param_ptr->type)
	{
		...
		case SENSOR_VAL_TYPE_READ_OTP_GAIN:
			rtn = _ov8825_read_otp_gain((uint32_t)param_ptr->pval);
		default:
			break;
	}
...
LOCAL uint32_t _ov8825_close_flag(unsigned long param)
{
	...
	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
			rtn = _ov8825_set_sensor_close_flag();
			break;
		default:
			break;
	}
...
LOCAL unsigned long _ov8825_StreamOff(unsigned long param)
{
	...
	value = Sensor_ReadReg(0x301c);
	if (value == 0xf0) {
		Sensor_WriteReg(0x301a, 0x71);
		Sensor_WriteReg(0x301c, 0xf4);
		if (!s_ov8825_sensor_close_flag) {
			sleep_time = 100*1000;
			usleep(sleep_time);
		}
	} else {
