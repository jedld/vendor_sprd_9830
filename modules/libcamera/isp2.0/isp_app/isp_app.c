...
int LSC_SPD_VERSION = 1;  // 1. smart , 2. smart + auto1.0 , 3. HLSC
int lsc_notice_flash_mode = 0;
...
{
	// system setting
	lsc_param->lsc2_tune_param.LSC_SPD_VERSION = 3;     // 1. smart , 2. smart + auto1.0 , 3. HLSC2.0
	LSC_SPD_VERSION = lsc_param->lsc2_tune_param.LSC_SPD_VERSION;  
	lsc_param->lsc2_tune_param.number_table = 5;

	// control_param
	lsc_param->lsc2_tune_param.alg_mode = 8;
	lsc_param->lsc2_tune_param.table_base_index = 1;  // 0. Alight  1. TL84  2. CWF  3. D65  4. DNP
	lsc_param->lsc2_tune_param.flash_index = 3;
	lsc_param->lsc2_tune_param.freq = 3;                
	lsc_param->lsc2_tune_param.IIR_weight = 5;   // [0 ~ 16]

	// smart_lsc2.0 param
	lsc_param->lsc2_tune_param.num_seg_queue = 10;
	lsc_param->lsc2_tune_param.num_seg_vote_th = 8;
	lsc_param->lsc2_tune_param.IIR_smart2 = 2;    // [0 ~ 16]

	// Auto_lsc1.0 param
	lsc_param->lsc2_tune_param.strength = 5;

	// Auto_lsc2.0 param
	lsc_param->lsc2_tune_param.lambda_r = 0.00005;
	lsc_param->lsc2_tune_param.lambda_b = 0.00005;
	lsc_param->lsc2_tune_param.weight_r = 2;
	lsc_param->lsc2_tune_param.weight_b = 2;

    //alg0.c
	lsc_param->tune_param.strength_level = lsc_param->lsc2_tune_param.strength;
	lsc_param->tune_param.con_weight = lsc_param->lsc2_tune_param.IIR_weight;
	lsc_param->tune_param.freq = lsc_param->lsc2_tune_param.freq;
	lsc_param->tune_param.pa = 1;
	lsc_param->tune_param.pb = 1;
	lsc_param->tune_param.fft_core_id = 0;	
	lsc_param->tune_param.restore_open = 0;	
...
uint32_t _alsc_init(isp_ctrl_context* handle)
{
	...
	_alsc_set_param(&lsc_param);
	if(LSC_SPD_VERSION <= 2 ){
		lsc_param.alg_open = 0;
	}else{
...
if(LSC_SPD_VERSION >= 2){
	...
	calc_param.flash_mode = lsc_notice_flash_mode;
	...
	case ISP_PROC_EVT_AE:
	{
		...
		if (ISP_SUCCESS == ae_rtn) {
			...
			if ((1 == handle->isp_smart_eb) && (0 == gCntSendMsgLsc % 3)) {
				...
				if(NULL==msg.data){ALOGE("ALSC2 msg to lsc thread failed");}
