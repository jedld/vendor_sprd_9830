...
static int32_t af_get_debug_info(struct af_context_t *af_cxt,void *param1)
{
	int32_t rtn = AF_SUCCESS;
...
static int32_t af_get_debug_info_for_display(struct af_context_t *af_cxt,void *param1)
{
	int32_t rtn = AF_SUCCESS;
	struct debug_af_display_param *emParam = (struct debug_af_display_param*)param1;
				...
				void *param0, void *param1)
{
	...
	case AF_CMD_GET_AF_CUR_POS:
		*(uint32_t*)param0 = af_cxt->cur_af_pos;
		break;
	case AF_CMD_GET_DEBUG_INFO:
		if (param1) {
			rtn = af_get_debug_info(af_cxt,param1);
		}
		break;
	case AF_CMD_GET_EM_PARAM:
		rtn = af_get_debug_info_for_display(af_cxt, param1);
		break;
	default:
