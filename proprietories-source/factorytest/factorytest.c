...
static int show_phone_test_result(void)
{
	...
	pcba_phone=0;
	num = sizeof(mmi_data_table)/sizeof(mmi_data_table[0])-K_MENU_NOT_AUTO_TEST_CNT;
	while(1) {
		menu_cnt = 0;
		for(i = 0; i < num; i++){
...
static int show_suggestion_test_result(void)
{
	...
	while(1) {
		menu_cnt = 0;
		for(i = 0; i < K_MENU_NOT_AUTO_TEST_CNT; i++){
             id = mmi_data_table[num-K_MENU_NOT_AUTO_TEST_CNT+i].id;
			 LOGD("CASE_TEST_TEL = %d, ID = %d, NUM= %d", CASE_TEST_TEL, id, num);
			 if(!test_case_support(id))
				continue;
                    ptr = get_result_ptr(id);
                    memset(tmp[menu_cnt], 0, sizeof(tmp[menu_cnt]));
                    switch(ptr->pass_faild) {
				case RL_NA:
					sprintf(tmp[menu_cnt],"%s:%s",(mmi_data_table[num-K_MENU_NOT_AUTO_TEST_CNT+i].name+2),TEXT_NA);
					break;
				case RL_FAIL:
					sprintf(tmp[menu_cnt],"%s:%s",(mmi_data_table[num-K_MENU_NOT_AUTO_TEST_CNT+i].name+2),TEXT_FAIL);
					break;
				case RL_PASS:
					sprintf(tmp[menu_cnt],"%s:%s",(mmi_data_table[num-K_MENU_NOT_AUTO_TEST_CNT+i].name+2),TEXT_PASS);
					break;
				case RL_NS:
					sprintf(tmp[menu_cnt],"%s:%s",(mmi_data_table[num-K_MENU_NOT_AUTO_TEST_CNT+i].name+2),TEXT_NS);
					break;
				default:
					sprintf(tmp[menu_cnt],"%s:%s",(mmi_data_table[num-K_MENU_NOT_AUTO_TEST_CNT+i].name+2),TEXT_NA);
					break;
                    ...
                    menu_not_suggestion_result_menu[menu_cnt].func= mmi_data_table[num-K_MENU_NOT_AUTO_TEST_CNT+i].func;
